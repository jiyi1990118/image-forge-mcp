...
> 通过 MCP 配置的 `env` 字段注入默认值。工具调用省略参数时自动使用。仅以下变量有效。旧版 `ENHANCE_DEFAULT_*` / `TEXT_SYSTEM` / `POLLINATIONS_TOKEN` / `POLLINATIONS_REFERRER` 已移除；`REALESRGAN_*` 服务于 `generateImage` 的默认增强路径。
...
| `BG_REMOVAL_TIMEOUT_MS` | `300000` | ONNX 抠图超时（毫秒）。首次运行会下载 ~170MB 模型，网络慢时可调大 |

## Real-ESRGAN（可选，用于 generateImage 默认增强）
...
| `REALESRGAN_RECHECK_MS` | `300000` | Real-ESRGAN 不可用时重新探测的间隔（毫秒）。Vulkan 驱动后加载时可在下次调用恢复 |
